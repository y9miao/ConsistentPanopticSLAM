...
MethodName = "Estimate/FinalVersion"
Task = "CoCoPano"
UseSemantic = 1


seqs = os.listdir(ResultsSeqsFolder)
method_name = "Estimate/Sem2Confidence4Fast"
seqs.sort()
results_folders = [os.path.join(ResultsSeqsFolder, seq, method_name) for seq in seqs]

exist_evaluation_seqs = []
lack_evaluation_seqs = []
error_result = []

for seq_i, result in enumerate(results_folders):

    # cehck results exist
    mAP_f = os.path.join(result, "geo_sem_eval", "mAP.pkl")
    panoptic_f = os.path.join(result, "geo_sem_eval", "panoptic_quality.pkl")
    chamfer_f = os.path.join(result, "geo_sem_eval", "chamfer.pkl")
    gt_f = os.path.join(result, "geo_sem_eval", "gt_class_counts.pkl")

    label_proj_f  = os.path.join(result, "geo_sem_eval", "label_mesh_proj.ply")
    semantic_proj_f = os.path.join(result, "geo_sem_eval", "semantic_mesh_proj.ply")
    inst_proj_f = os.path.join(result, "geo_sem_eval", "instance_mesh_proj.ply")

    label_f = os.path.join(result, "label_mesh_.ply")
    semantic_f = os.path.join(result, "semantic_mesh_.ply")
    inst_f = os.path.join(result, "instance_mesh_.ply")

    is_mAP = os.path.isfile(mAP_f)
    is_panoptic = os.path.isfile(panoptic_f)
    is_chamfer = os.path.isfile(chamfer_f)
    is_gt = os.path.isfile(gt_f)

    is_label_proj_f = os.path.isfile(label_proj_f)
    is_semantic_proj_f = os.path.isfile(semantic_proj_f)
    is_inst_proj_f = os.path.isfile(inst_proj_f)

    is_label_f = os.path.isfile(label_f)
    is_semantic_f = os.path.isfile(semantic_f)
    is_inst_f = os.path.isfile(inst_f)

    # if(is_mAP and is_panoptic and is_chamfer and is_gt and is_inst_proj_f):
    if(is_mAP and is_panoptic and is_chamfer):
        exist_evaluation_seqs.append(seqs[seq_i])
        # # remove mesh
        # label_mesh_f = os.path.join(result, "label_mesh_.ply")
        # semantic_mesh_f = os.path.join(result, "semantic_mesh_.ply")
        # inst_mesh_f = os.path.join(result, "instance_mesh_.ply")
        # command = "rm " + str(label_mesh_f) + " && " + "rm  " + str(semantic_mesh_f) + " && " + \
        #     "rm  " + str(inst_mesh_f)
        # subprocess.call(command, shell=True)
        pass
    else:
        lack_evaluation_seqs.append(seqs[seq_i])


evaluateBatch(MetricFolder, ResultsSeqsFolder, exist_evaluation_seqs, MethodName)