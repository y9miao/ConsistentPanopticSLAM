...
def evaluate_matches(matches, use_confidence=False):
    ...
    chamfer_matches = {}
    panoptic_quality = {}
    gt_class_counts = {class_label:0 for class_label in CLASS_LABELS}
    matched_instance_num = {}
    ...
    for di, (min_region_size, distance_thresh, distance_conf) in enumerate(zip(min_region_sizes, dist_threshes, dist_confs)):
        for oi, overlap_th in enumerate(overlaps):
            ...
            panoptic_quality[overlap_th_perc] = {'count':{class_label:0 for class_label in CLASS_LABELS}}
            matched_instance_num[overlap_th_perc] = {class_label:0 for class_label in CLASS_LABELS}
            ...
            for li, label_name in enumerate(CLASS_LABELS):
                        ...
                        for pred in gt['matched_pred']:
                            ...
                            if overlap > overlap_th:
                                ...
                                # otherwise set score
                                    ...
                                    # add mathced instance num 
                                    matched_instance_num[overlap_th_perc][label_name] += 1
                        if not found_match:
                ...
                # initialize panoptoc qiality
                if has_gt:
                    panoptic_quality[overlap_th_perc][label_name]["RQ"] = 0.
                    panoptic_quality[overlap_th_perc][label_name]["PQ"] = 0.
                ...
                if has_gt and has_pred:
                    ...
                    for idx_res,idx_scores in enumerate(unique_indices):
                        ...
                        if(idx_res==0):
                            ...
                            panoptic_quality[overlap_th_perc]['count'][label_name] = tp
                            pass
    ...
    return ap, chamfer_matches, gt_class_counts, panoptic_quality, matched_instance_num
...
def calculate_chamfer_distance_cuda(chamfer_matches, mesh_f, device):
    ...
    for ov in [25, 50, 75]:
        chamfer_matches_ov = chamfer_matches[ov]
        chamfer_distances_ov = {}
        chamfer_matched_num_ov = { class_name: 0 for class_name in CLASS_LABELS}
        for instance_id in chamfer_matches_ov:
            if gt_labels is None:
                gt_labels_f = chamfer_matches_ov[instance_id]['gt_file']
                gt_labels = np.load(gt_labels_f).reshape(-1)
            ...
            chamfer_distances_ov[class_name]['pred2gt'].append(dist_pred2gt)
            chamfer_distances_ov[class_name]['gt2pred'].append(dist_gt2pred)
            chamfer_matched_num_ov[class_name] += 1
        ...
        chamfer_distances_result[ov]['pred2gt'] = { class_name: np.nan for class_name in CLASS_LABELS}
        chamfer_distances_result[ov]['gt2pred'] = { class_name: np.nan for class_name in CLASS_LABELS}
        chamfer_distances_result[ov]['count'] = copy.deepcopy(chamfer_matched_num_ov)
        for class_name in CLASS_LABELS:
            if class_name in chamfer_distances_ov and len(chamfer_distances_ov[class_name]['pred2gt'])!=0:
                chamfer_distances_result[ov]['pred2gt'][class_name] = np.average(np.array(chamfer_distances_ov[class_name]['pred2gt']))
            if class_name in chamfer_distances_ov and len(chamfer_distances_ov[class_name]['gt2pred'])!=0:
...
def write_result_file(avgs, chamfer_distance_class, output_folder, evaluation_folder, \
        gt_class_counts, panoptic_quality, matched_instance_num):
    _SPLITTER = ','
    import pickle
    ...
    matched_instance_num_txt = os.path.join(output_folder, "matched_instance_num.txt")
    ...
    # with open(gt_class_counts_txt, 'w') as f:
    #     f.write(_SPLITTER.join(['class'.ljust(10), 'gt_count'.ljust(3)]) + '\n')
    #     for i in range(len(VALID_CLASS_IDS)):
    #         class_name = CLASS_LABELS[i]
    #         f.write(_SPLITTER.join([str(class_name).ljust(10), str(gt_class_counts[class_name]).ljust(3)]) + '\n') 
    # save matched_instance_num results
    with open(matched_instance_num_txt, 'w') as f:
        ov_th_list = list(matched_instance_num.keys())
        ov_th_list.sort()
        head_string = _SPLITTER.join(['class'.ljust(10), 'gt'.ljust(3)] + \
            [ str(ov_th).ljust(3) for ov_th in ov_th_list])
        f.write(head_string + '\n')
        for class_name in CLASS_LABELS:
            if gt_class_counts[class_name] == 0:
                continue
            class_string = [str(class_name).ljust(10), str(gt_class_counts[class_name]).ljust(3)]
            for ov_th in ov_th_list:
                value = matched_instance_num[ov_th][class_name]
                value_str = format(value).ljust(3)[:3]
                class_string.append(value_str)
            f.write(_SPLITTER.join(class_string) + '\n')
    with open(matched_instance_num_pkl, 'wb') as f:
        pickle.dump(matched_instance_num, f)

    # save mAP results
    with open(mAP_txt, 'w') as f:
        f.write(_SPLITTER.join(['class'.ljust(12), 'class id'.ljust(8), 'ap75'.ljust(10), 
        ...
        for i in range(len(VALID_CLASS_IDS)):
            ...
            f.write(_SPLITTER.join([str(x) for x in [str(class_name).ljust(12), str(class_id).ljust(8), ap75, ap50,ap25]]) + '\n') 

        ap75 = format(avgs["all_ap"], '.10f').ljust(10)[:10]
        ap50 = format(avgs["all_ap_50%"], '.10f').ljust(10)[:10]
        ap25 = format(avgs["all_ap_25%"], '.10f').ljust(10)[:10]
        f.write(_SPLITTER.join([str(x) for x in ["ave".ljust(12), ap75,ap50, ap25]]) + '\n') 
    ...
    with open(chamfer_txt, 'w') as f:
        f.write(_SPLITTER.join(['class'.ljust(12), 'class id'.ljust(8),  
                'pred_ov50'.ljust(8),'gt_ov50'.ljust(8),'num_ov50'.ljust(8),'pred_ov75'.ljust(8),'gt_ov75'.ljust(8),'num_ov75'.ljust(8) ]) + '\n')
        for i in range(len(VALID_CLASS_IDS)):
            class_name = CLASS_LABELS[i]
            class_id = VALID_CLASS_IDS[i]
            ...
            num_ov50 = format(chamfer_distance_class[50]['count'][class_name]).ljust(8)[:8]
            num_ov75 = format(chamfer_distance_class[75]['count'][class_name]).ljust(8)[:8]
            f.write(_SPLITTER.join([str(class_name).ljust(12), str(class_id).ljust(8), 
                pred_ov50,gt_ov50 , num_ov50,pred_ov75, gt_ov75, num_ov75]) + '\n')    
    ...
    with open(panoptic_quality_txt, 'w') as f:
        f.write(_SPLITTER.join(['class'.ljust(12), 'class id'.ljust(8),  
                'PQ_50'.ljust(6),'SQ_50'.ljust(6),'RQ_50'.ljust(6),'num_ov50'.ljust(8),
                'PQ_75'.ljust(6),'SQ_75'.ljust(6),'RQ_75'.ljust(6),'num_ov75'.ljust(8) ]) + '\n')
        for i in range(len(VALID_CLASS_IDS)):
            class_name = CLASS_LABELS[i]
            class_id = VALID_CLASS_IDS[i]
            PQ_50 = format(panoptic_quality[50][class_name]["PQ"], '.5f').ljust(6)[:6]
            SQ_50 = format(panoptic_quality[50][class_name]["SQ"], '.5f').ljust(6)[:6]
            RQ_50 = format(panoptic_quality[50][class_name]["RQ"], '.5f').ljust(6)[:6]
            num_ov50 = format(panoptic_quality[50]["count"][class_name]).ljust(8)[:8]
            PQ_75 = format(panoptic_quality[75][class_name]["PQ"], '.5f').ljust(6)[:6]
            SQ_75 = format(panoptic_quality[75][class_name]["SQ"], '.5f').ljust(6)[:6]
            RQ_75 = format(panoptic_quality[75][class_name]["RQ"], '.5f').ljust(6)[:6]
            num_ov75 = format(panoptic_quality[75]["count"][class_name]).ljust(8)[:8]
            f.write(_SPLITTER.join([str(x) for x in [str(class_name).ljust(12), str(class_id).ljust(8), 
                PQ_50, SQ_50, RQ_50,num_ov50, PQ_75, SQ_75, RQ_75, num_ov75]]) + '\n')    
...
def evaluate(pred_files, gt_files, pred_path, output_folder, evaluation_folder, gt_mesh_f, use_confidence=False):
    ...
    for i in range(len(pred_files)):
        ...
        sys.stdout.flush()
    print('')
    ap_scores, chamfer_matches,gt_class_counts, panoptic_quality,matched_instance_num = evaluate_matches(matches, use_confidence)
    ...
    if(o3c.cuda.is_available()):
        ...
        print(" Sorry! No cuda available!!! ")
        chamfer_distances = calculate_chamfer_distance(chamfer_matches, gt_mesh_f)
    # print
    # print_results(avgs)
    write_result_file(avgs, chamfer_distances, output_folder,evaluation_folder, gt_class_counts, panoptic_quality, matched_instance_num)
