...
void Controller::integrateFrame(ros::Time msg_timestamp) {
  ...
  if(integrated_frames_count_ == 1) time_first_frame = msg_timestamp;
  LOG(INFO) << std::fixed << "Integrating frame n." << ++integrated_frames_count_
            << ", timestamp of frame: " << (msg_timestamp.toSec()-time_first_frame.toSec());
