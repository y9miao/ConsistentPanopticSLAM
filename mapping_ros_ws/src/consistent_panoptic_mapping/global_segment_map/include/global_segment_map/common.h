...
extern uint8_t BackgroundSemLabel;
// Voxblox++ custom types.
typedef uint16_t Label;
// typedef uint16_t LabelConfidence;
typedef float LabelConfidence;
typedef float LabelProbability;
typedef float SegmentObserveConfidence;
typedef float GeometricConfidence;
typedef float SegSegConfidence;
typedef float ObjSegConfidence;
...
typedef std::map<SemanticLabel, SegmentObserveConfidence> SLMap;
typedef std::map<Label, SLMap> LSLMap;

#define VoxelLabelArrayLen 3
struct LabelCount {
  Label label = 0u;
  LabelConfidence label_confidence = 0;
};
