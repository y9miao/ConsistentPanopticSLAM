...
void LabelTsdfIntegrator::checkForSegmentLabelMergeCandidate(
    const Label& label, const LabelConfidence label_points_count,
...
Label LabelTsdfIntegrator::getNextUnassignedLabel(
  ...
  if (label_it != assigned_labels.end()) {
    ...
    LabelConfidence max_confidence = 0.;
...
void LabelTsdfIntegrator::addVoxelLabelConfidence(
  ...
  if (updated == false) {
    ...
    // find label in label_count with least label_confidence

    // DONE(MY): new label substitute labels with least confidence
    int mini_confidence_i = 0;
    LabelConfidence mini_confidence = 1e6;
    for (int l_c_i=0; l_c_i<VoxelLabelArrayLen; l_c_i++) {
        if(mini_confidence > label_voxel->label_count[l_c_i].label_confidence){
          mini_confidence = label_voxel->label_count[l_c_i].label_confidence;
          mini_confidence_i = l_c_i;
        }
    }
    if(mini_confidence < confidence){
      label_voxel->label_count[mini_confidence_i].label = label;
      label_voxel->label_count[mini_confidence_i].label_confidence = mini_confidence;
    }
...
// CHANGE_MY when merging labels, use max length instead of summation
void LabelTsdfIntegrator::addPairwiseConfidenceCountMaximum(
    const LLMapIt& label_map_it, const Label& label, const int count) {
  LMapIt label_count_it = label_map_it->second.find(label);
  if (label_count_it != label_map_it->second.end()) {
    // label_map already contains a pairwise confidence count
    // to label, increase existing value by count.
    int count_to_add = std::max(label_count_it->second, count);
    label_map_it->second[label] = count_to_add;
  } else {
    label_map_it->second.emplace(label, count);
  }
}
...
void LabelTsdfIntegrator::mergeLabels(LLSet* merges_to_publish) {
  ...
  if (label_tsdf_config_.enable_pairwise_confidence_merging) {
    ...
    while (getNextMerge(&new_label, &old_label)) {
      ...
      // Color new_label_color_label = 
      LOG(INFO) << "Merging labels " << new_label << " and " << old_label;
      ...
      // LOG(INFO) << "Merging labels " << new_label <<" with semantic label " << int(new_label_semantic)
      //           << " and " << old_label <<" with semantic label " << int(old_label_semantic);
      ...
      LOG(INFO) << "  After merging, semantics becomes " << int(new_label_semantic);
      merge_timer.Stop();
