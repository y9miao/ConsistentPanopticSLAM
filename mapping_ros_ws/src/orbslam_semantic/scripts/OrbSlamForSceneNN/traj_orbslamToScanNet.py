...
def parse_args():
    ...
    parse.add_argument("--traj_gt", type=str,required=True,help=" input")
...
def main():
    ...
    traj_gt_folder = os.path.join(scene_folder, "trajectory.log")
    len_poses_gt = 0
    with open(traj_gt_folder, 'r') as gt_f:
        lines = gt_f.read().splitlines()
        len_poses_gt = len(lines)
    traj_gt = logToTraj(traj_gt_folder)
    ...
    T_GT_C = None
    while True:
        if(initial_idx in traj_gt and initial_idx in traj_orb):
            T_GT_C = traj_gt[initial_idx] @ np.linalg.inv(traj_orb[initial_idx])
            break
        initial_idx += 1
        if(initial_idx > len_poses_gt):
            return 
    for frame_i in traj_orb:

        traj_orb[frame_i] = T_GT_C @ traj_orb[frame_i]
