                ...
                vector<string> &vstrImageFilenamesD, int img_num)
{
    ...
    std::string rgb_folder = sceneFolder + "/image/";

    int image_index = 0;
    size_t z_fill_len = 5;
    for(image_index = 0; image_index < img_num; image_index++)
    {
        std::string frame_idx_str = std::to_string(image_index+1);
        frame_idx_str = 
            std::string(z_fill_len - std::min(z_fill_len, frame_idx_str.length()), '0')
            + frame_idx_str;
        if(image_index==0) cout << "frame_idx_str: " <<   frame_idx_str << endl;
        std::string depth_f = depth_folder + "depth" + frame_idx_str + ".png";
        std::string rgb_f = rgb_folder + "image" + frame_idx_str + ".png";
