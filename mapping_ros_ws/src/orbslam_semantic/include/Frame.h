...
public:
    ...
    // 3D-2D pairs residual during pose optimization
    std::map<MapPoint*, float> mapPoints_residual_;
    // 3D-2D pairs inlier ratio
    float inlier_ratio_ = 0;
    int num_inliers_ = 0;
    // 3D-2D covariance matrix
