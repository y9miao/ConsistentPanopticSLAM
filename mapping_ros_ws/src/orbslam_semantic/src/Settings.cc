...
namespace ORB_SLAM3 {
    ...
    bNeedToUndistort_(false), bNeedToRectify_(false), bNeedToResize1_(false), bNeedToResize2_(false) {
        ...
        readTrackMode(fSettings);
        cout << "\t-Loaded Tracking Mode" << endl;

        readTrackDebug(fSettings);
        cout << "\t-Loaded Tracking Debug" << endl;
    ...
    void Settings::readTrackMode(cv::FileStorage &fSettings)
    {
        cv::FileNode node = fSettings["Tracking.mode"];
        if(!node.empty()){
    ...
    void Settings::readTrackDebug(cv::FileStorage &fSettings)
    {
        cv::FileNode node = fSettings["Tracking.debug"];
        if(!node.empty()){
            tracking_debug_ = ( (int)node != 0 );
